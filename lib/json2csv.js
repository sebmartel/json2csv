 ...
 * @return the csv.
 * Note: Errors are thrown if no callback is supplied
 */
module.exports = function (params, callback) {
  var csv, err = checkParams(params);
  if (!err) {
    var titles = createColumnTitles(params);
    csv = createColumnContent(params, titles);
  }
...
function checkParams(params) {
  ...
  if (params.fieldNames && params.fieldNames.length !== params.fields.length) {
    return new Error('fieldNames and fields should be of the same length, if fieldNames is provided.');
  }
